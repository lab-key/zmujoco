...
const SUBMODULE_URL = "https://github.com/lab-key/mujoco/";
const SUBMODULE_BRANCH = "dev";
const SUBMODULE_PATH = "libs/upstream/mujoco";
const VENDOR_PATH = "libs/mujoco";
...
pub fn main() !void {
    ...
    if (!try isSubmoduleTracked(allocator, SUBMODULE_PATH)) {
        ...
        try runCommand(allocator, &[_][]const u8{ "git", "submodule", "add", "-b", SUBMODULE_BRANCH, SUBMODULE_URL, SUBMODULE_PATH });
    } else {
    ...
    std.debug.print("Vendoring ccd from {s} to {s}...\n", .{ SUBMODULE_PATH, VENDOR_PATH });
    try copyDirRecursive(allocator, SUBMODULE_PATH, VENDOR_PATH);
